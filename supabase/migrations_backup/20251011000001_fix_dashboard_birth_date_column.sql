-- Fix dashboard RPC to use s.birth_date instead of u.birth_date
-- birth_date column is in students table, not users table
...
COMMENT ON FUNCTION get_dashboard_data(date) IS 'Get all dashboard data in a single RPC call with tenant isolation. Fixed to use s.birth_date instead of u.birth_date';
