-- search_textbooks_list 에 lending_count / unit_count 통합
--
-- 배경:
--   getTextbooks server action 이 RPC 호출 후 book_lendings, textbook_units
--   추가 fetch 를 별도 round-trip 으로 수행해서 검색 latency 가 ~100ms 추가됨.
--   RPC 결과 jsonb 에 count 를 LATERAL subquery 로 포함시켜 round-trip 절감.
--
-- 효과:
--   - 검색 한 번에 round-trip 3개 → 1개 (RPC 안에서 모두 처리)
--   - PostgREST 직렬화 / 네트워크 / 인증 overhead 가 그만큼 줄어듦
