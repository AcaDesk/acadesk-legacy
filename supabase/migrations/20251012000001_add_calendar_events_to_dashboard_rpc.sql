-- Update dashboard function to include calendar events
...
BEGIN
  ...
  -- Get start and end of current month for calendar
  start_of_month := date_trunc('month', today_param)::date;
  end_of_month := (date_trunc('month', today_param) + interval '1 month - 1 day')::date;
    ...
    'calendarEvents', (
      ...
      FROM (
        ...
        WHERE ce.tenant_id = current_tenant_id
          ...
          AND ce.start_time >= start_of_month::timestamptz
          AND ce.start_time < (end_of_month + interval '1 day')::timestamptz
...
COMMENT ON FUNCTION get_dashboard_data(date) IS 'Dashboard data function with calendar events support';