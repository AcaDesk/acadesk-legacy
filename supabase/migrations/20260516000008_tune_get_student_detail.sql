-- get_student_detail RPC 튜닝 (옵션 C — 안전한 개선)
--
-- 변경:
--   1. invoices 테이블이 존재하지 않으므로 BEGIN/EXCEPTION 블록 제거.
--      매 호출시 발생하던 undefined_table 예외 catch 비용 절감.
--   2. STABLE 마킹 — 트랜잭션 내 동일 입력 시 결과 불변. plan 캐시 활용.
--   3. SET search_path = pg_catalog, public — function_search_path_mutable 해소.
--   4. JSON 출력 구조 100% 동일 — 클라이언트 코드 영향 없음.
--
-- 호출 빈도: 576회 / 평균 33.67ms / 앱 쿼리 누적시간 57%.
