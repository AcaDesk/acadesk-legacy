/**
 * Supabase Student Repository Implementation
 * 학생 리포지토리의 Supabase 구현 - 인프라 레이어
 */
...
export class SupabaseStudentRepository implements IStudentRepository {
  ...
  async findById(id: string, options?: FindStudentOptions): Promise<Student | null> {
    try {
      ...
      if (error) {
        logError(error, { repository: 'SupabaseStudentRepository', method: 'findById', id })
      ...
      logError(error, { repository: 'SupabaseStudentRepository', method: 'findById' })
  ...
  async search(query: string, tenantId: string, limit: number = 20): Promise<Student[]> {
    try {
      ...
      if (error) {
        logError(error, { repository: 'SupabaseStudentRepository', method: 'search', query })
      ...
      if (error instanceof DatabaseError) throw error
      logError(error, { repository: 'SupabaseStudentRepository', method: 'search' })
  ...
  async delete(id: string): Promise<void> {
    try {
      ...
      if (error) {
        logError(error, { repository: 'SupabaseStudentRepository', method: 'delete', id })
        throw new DatabaseError('학생을 삭제할 수 없습니다', error)
      }
    } catch (error) {
      if (error instanceof DatabaseError) throw error
      logError(error, { repository: 'SupabaseStudentRepository', method: 'delete' })
