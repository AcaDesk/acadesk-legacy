...
export class SupabaseTodoRepository implements ITodoRepository {
  ...
  async findById(id: string): Promise<Todo | null> {
    try {
      ...
      if (error) {
        logError(error, { repository: 'SupabaseTodoRepository', method: 'findById', id })
      ...
      logError(error, { repository: 'SupabaseTodoRepository', method: 'findById' })
  ...
  async findByStudentId(studentId: string, includeCompleted: boolean = true): Promise<Todo[]> {
    try {
      ...
      if (error) {
        logError(error, { repository: 'SupabaseTodoRepository', method: 'findByStudentId' })
  ...
  async findAll(tenantId: string, filters?: TodoFilters, limit: number = 100): Promise<Todo[]> {
    try {
      ...
      if (error) {
        logError(error, { repository: 'SupabaseTodoRepository', method: 'findAll' })
  ...
  async findUpcoming(studentId: string, days: number = 3): Promise<Todo[]> {
    try {
      ...
      if (error) {
        logError(error, { repository: 'SupabaseTodoRepository', method: 'findUpcoming' })
  ...
  async findOverdue(studentId: string): Promise<Todo[]> {
    try {
      ...
      if (error) {
        logError(error, { repository: 'SupabaseTodoRepository', method: 'findOverdue' })
  ...
  async save(todo: Todo): Promise<Todo> {
    try {
      ...
      if (error) {
        logError(error, { repository: 'SupabaseTodoRepository', method: 'save' })
  ...
  async saveBulk(todos: Todo[]): Promise<Todo[]> {
    try {
      ...
      if (error) {
        logError(error, { repository: 'SupabaseTodoRepository', method: 'saveBulk' })
  ...
  async delete(id: string): Promise<void> {
    try {
      ...
      if (error) {
        logError(error, { repository: 'SupabaseTodoRepository', method: 'delete', id })
        throw new DatabaseError('TODO를 삭제할 수 없습니다', error)
      }
    } catch (error) {
      if (error instanceof DatabaseError) throw error
      logError(error, { repository: 'SupabaseTodoRepository', method: 'delete' })
  ...
  async getStats(filters?: { studentId?: string; tenantId?: string }): Promise<TodoStats> {
    try {
      ...
      if (error) {
        logError(error, { repository: 'SupabaseTodoRepository', method: 'getStats' })
  ...
  async getCompletionRate(studentId: string, dateFrom?: Date, dateTo?: Date): Promise<number> {
    try {
      ...
      if (error) {
        logError(error, { repository: 'SupabaseTodoRepository', method: 'getCompletionRate' })
