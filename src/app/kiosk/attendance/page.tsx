...
type Step = 'setup_required' | 'input' | 'confirm' | 'success' | 'admin_auth'
...
export default function KioskAttendancePage() {
  ...
  async function handlePinSubmit(submittedPin: string) {
    ...
    const result = await lookupStudentByPin(tenantId, submittedPin)
    setIsLoading(false)

    if (!result.success || !result.student) {
      setError(result.error || 'PIN을 찾을 수 없습니다.')
      setPin('')
      return
    }
  ...
  function handleAdminPinSubmit(submittedPin: string) {
    const exitPin =
      typeof window !== 'undefined'
        ? (localStorage.getItem('kiosk_exit_pin') ?? '9999')
        : '9999'
      ...
      <div className="min-h-screen flex items-center justify-center bg-background">
        <div className="text-center space-y-4 p-8">
          ...
          <p className="text-muted-foreground">
            키오스크를 사용하려면 먼저 설정을 완료해주세요.
          </p>
          <Button size="lg" onClick={() => router.push('/kiosk/setup')}>
            설정하러 가기
          </Button>
        </div>
      </div>
        ...
        {step === 'input' && (
          ...
          >
            ...
            <div className="text-center space-y-1">
              <h2 className="text-xl font-semibold">PIN 번호를 입력해주세요</h2>
              <AnimatePresence mode="wait">
                {error && (
                  <motion.p
                    key={error}
